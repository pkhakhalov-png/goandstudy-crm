...
async function insertProgram(prog: any) {
  ...
  const r = await parser.from('programs').insert(prog).select('id, name, school_id').single().then(warnOnError('programs · scripts/add-se-fi-it-programs.ts:136'))
...
async function ensureSchool(school: any, matchLike: string): Promise<number> {
  ...
  const r = await parser.from('schools').insert(school).select('id, name').single().then(warnOnError('schools · scripts/add-se-fi-it-programs.ts:144'))
