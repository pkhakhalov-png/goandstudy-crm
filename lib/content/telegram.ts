...
export class TelegramConnector implements PublishingConnector {
  ...
  /**
   * Жив ли пост сейчас (E5.14).
   *
   * Три состояния, которые PRD велит различать, здесь различаются так:
   *
   *   «API принял» — у нас есть message_id. Это ВСЁ, что он означает.
   *   «Видно в аккаунте» — проверить нечем: getMessage в Bot API нет.
   *   «Доступно публично» — проверяется единственным способом, который
   *     работает: встраиваемым видом публичной страницы поста.
   *
   * Важная тонкость, из-за которой наивная проверка врёт. Обычный адрес
   * t.me/канал/номер отдаёт одну и ту же карточку канала и для настоящего
   * поста, и для несуществующего номера — проверено на живых адресах, разница
   * в разметке нулевая. Отличает их только `?embed=1`: у несуществующего в
   * ответе `tgme_widget_message_error` и «Post not found».
   */
  async getStatus(remoteId: string): Promise<RemoteStatus> {
    if (!this.имяКанала) {
      return {
        kind: 'не_проверить',
        why: `Канал приватный: публичной страницы у поста #${remoteId} нет, `
          + 'а Bot API не даёт запросить сообщение повторно — getMessage не существует.',
      }
    }
    const url = ссылкаНаПост(this.имяКанала, remoteId)
    if (!url) return { kind: 'не_проверить', why: 'адрес поста не построился' }

    try {
      const ctrl = new AbortController()
      const timer = setTimeout(() => ctrl.abort(), 10_000)
      const res = await fetch(`${url}?embed=1`, { signal: ctrl.signal, cache: 'no-store' })
      clearTimeout(timer)
      if (!res.ok) return { kind: 'не_проверить', why: `публичная страница ответила HTTP ${res.status}` }
      return разобратьСтраницуПоста(await res.text(), url)
    } catch (e: any) {
      // Не достучались — это «не проверили», а не «исчез». Разница
      // принципиальная: по «исчез» кто-нибудь пойдёт публиковать заново.
      return {
        kind: 'не_проверить',
        why: `публичная страница не открылась: ${String(e?.name === 'AbortError' ? 'таймаут' : e?.message ?? e)}`,
      }
    }
  }
 ...
 *
 * Вынесено отдельно и экспортируется: разбор проверяется на сохранённых
 * ответах, без сети.
 */
export function разобратьСтраницуПоста(html: string, url: string): RemoteStatus {
  if (/tgme_widget_message_error|Post not found/i.test(html)) return { kind: 'исчез' }
  // Дата поста со ссылкой на него есть только у настоящего поста. Карточка
  // канала, которую отдают вместо несуществующего, её не содержит.
  if (/tgme_widget_message_date/.test(html)) return { kind: 'на_месте', remoteUrl: url }
  // Ни того, ни другого — разметка изменилась. Это «не проверили», а не «нет»:
  // объявлять пост исчезнувшим из-за смены вёрстки нельзя.
  return { kind: 'не_проверить', why: 'разметка страницы не опознана — возможно, изменилась' }
}
