...
const KIND_RULES: { kind: TxKind; words: RegExp }[] = [
  ...
  { kind: 'income', words: word('пришл(о|а)', 'поступил(о|а)?', 'приход', 'оплат(а|ы|у)\\s+клиент\\p{L}*', 'получили\\s+от', 'заплатил(и|а)?\\s+(нам|клиент\\p{L}*)') },
  { kind: 'expense', words: word('расход', 'оплатил(а|и)?', 'потратил(а|и)?', 'купил(а|и)?', 'заплатил(а|и)?', 'зарплат(а|ы|у|е)', 'выплатил(а|и)?') },
]
