...
- Замечание про непроверенный факт закрывается УДАЛЕНИЕМ утверждения. Смягчить,
  добавить «обычно» или «как правило» — не исправление: утверждение остаётся,
  а проверить его по-прежнему нечем. Либо убираешь предложение целиком, либо
  заменяешь обобщением без числа и без ссылки на нашу практику.
- Отдельно строго: любые утверждения о goandstudy — «мы замечаем», «самая частая
  ошибка наших студентов», «по нашему опыту», доли и количества клиентов —
  удаляются полностью, если их нет в блоке фактов. Такое нельзя смягчить, потому
  что это выдумка о живой компании, а не стилистическая неточность.
- Цитата в замечании — точное место. После правки этой фразы в тексте быть не
...
/**
 * Правка одного фрагмента статьи, а не статьи целиком.
 *
 * Нужна для архива. Живая работающая страница — ценность: у неё есть позиции,
 * ссылки и читатели, и переписать её ради трёх непроверяемых цифр значит
 * поменять известное на неизвестное. Первый живой прогон правки архива это и
 * показал: писателю отдали статью целиком с двенадцатью замечаниями, а вернулась
 * статья на 18% длиннее, где дословно уцелело 36% предложений и появились два
 * новых раздела. Замечания он закрыл — и заодно переписал текст, которого никто
 * не просил трогать.
 *
 * Поэтому здесь ему нечего переписывать: на входе один абзац или список, на
 * выходе он же. Остальная статья к модели вообще не попадает и остаётся
 * побайтово прежней — это гарантируется не инструкцией, а тем, что её нет в
 * запросе.
 */
export async function reviseFragment(
  ctx: GenContext,
  fragment: string,
  ...
  const msg = await askModelStream(ctx, 'writer', {
    max_tokens: 4000,
    system: `Ты правишь ОДИН фрагмент опубликованной статьи под замечания проверки.

Правила:
- Возвращаешь тот же фрагмент с теми же тегами верхнего уровня. Абзац остаётся
  абзацем, список — списком с тем же числом пунктов, заголовок — заголовком.
- Меняешь только то, что названо в замечаниях. Соседние предложения оставляешь
  дословно — они не проверялись и трогать их незачем.
- Непроверяемое утверждение закрывается УДАЛЕНИЕМ. Смягчить, добавить «обычно»
  или «как правило» — не исправление: проверить утверждение от этого не станет
  чем, а читатель по-прежнему примет его за факт. Либо убираешь предложение
  целиком, либо заменяешь обобщением без числа.
- Ничего не добавляешь: ни новых мыслей, ни примеров, ни призывов. Фрагмент
  после правки должен стать короче или примерно той же длины.
- Если после удаления во фрагменте не остаётся содержания, верни пустую строку.
- Ответ — только HTML фрагмента, без markdown-обёрток и пояснений.`,
    messages: [{
      role: 'user',
      content: `ФРАГМЕНТ\n${fragment}\n\nЗАМЕЧАНИЯ\n${list}`,
    }],
...
- facts — числа, цены, сроки, требования, статистика, которых нет в блоке фактов и не следует
  ...
  Не считается замечанием: обязательный финальный призыв записаться на бесплатную
  консультацию. Бесплатная консультация — существующая услуга, отдельная от платной
  экспертной сессии, и прайсу она не противоречит.
