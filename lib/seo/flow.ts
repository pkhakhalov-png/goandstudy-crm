...
/**
 * Выбрать время следующей публикации — по аудитории, а не наугад.
 *
 * Было: случайный момент внутри окна, с обоснованием «одинаковое время изо дня
 * в день выдаёт машину». Обоснование не выдерживает проверки. Читателю всё
 * равно, в котором часу вышла статья; ему важно, увидит ли он её вообще. А
 * маскировка под живого редактора не работает и не нужна: поисковику безразличен
 * час выпуска, а человеку — тем более. Платой за маскировку был разброс
 * 09:06, 13:46, 14:10, 15:34, 16:58 — то есть половина статей выходила тогда,
 * когда аудитории нет.
 *
 * Стало: целимся в час, когда нашу аудиторию застать вероятнее всего.
 *
 * ВАЖНО про происхождение окна. Данных по часам у нас пока нет: Search Console
 * даёт дни, а не часы, и собственная статистика переходов только начала
 * копиться. Значение ниже — предположение, а не измерение, и помечено как
 * предположение намеренно. Проверить его можно будет по attribution_events,
 * когда наберётся месяц: тогда окно станет настройкой, выведенной из фактов.
 *
 * Разброс внутри часа остаётся, но по другой причине: две статьи не должны
 * претендовать на одну минуту. Это разведение слотов, а не маскировка.
 */
export function planNextPublish(settings: FlowSettings, after = new Date()): Date {
  ...
  // Целевой час — ближе к концу окна: вечер по Москве, когда абитуриент и
  // родитель освободились. Если окно короткое, берём его середину.
  const target = to - from >= 3 ? to - 2 : Math.floor((from + to) / 2)
  const hour = Math.min(Math.max(target, from), to)

  // Минуты внутри часа — чтобы слоты не совпадали, а не чтобы спрятаться.
  const jitterMin = Math.floor(Math.random() * 40)

  const at = day.getTime() + hour * 3600 * 1000 + jitterMin * 60 * 1000
