...
import { activateCuratorAccount } from './actions'
...
export function CuratorInviteForm({ token, email }: { token: string; email: string }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true)
    const res = await activateCuratorAccount({ token, password })
    setLoading(false)
  ...
  return (
    <form onSubmit={submit} style={cardStyle}>
      ...
      <p style={{ fontSize: 14, color: '#8a8796', textAlign: 'center', margin: '0 0 28px', lineHeight: 1.5 }}>
        Придумай пароль для входа. Email привязан администратором.
      </p>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <input id="pw" type="password" value={password} onChange={e => setPassword(e.target.value)}
          minLength={8} required autoFocus style={inputStyle} />
      </div>
