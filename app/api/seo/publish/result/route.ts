...
export async function POST(req: NextRequest) {
  ...
  // Страница — в инвентарь, сразу. Раньше строки в `pages` заводил только ручной
  // обход сайта, и до него ответ Google о статье было некуда записать: проверка
  // индексации молча пропускала статью, а экран показывал снимок недельной
  // давности. Апсерт идёт по адресу, поэтому повторный отчёт агента не задвоит.
  try {
