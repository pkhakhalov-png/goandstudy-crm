...
export default async function SalesCabinetPage() {
  ...
  // Считаем от НАЧАЛА встречи, а не от конца: отметить исход нужно по ходу
  // разговора или сразу после, пока помнишь. Ждать два часа после окончания
  // значило бы вспоминать вечером, чем кончилась встреча в одиннадцать утра, —
  // а именно из-за этого треть консультаций и осталась неотмеченной.
  //
  // `outcome_prompt_hours` оставлен настройкой: ноль — показывать с начала
  // встречи, положительное число — сдвинуть на столько часов позже.
  const порогМс = now.getTime() - запасЧасов * 3600_000
  const pendingOutcome = (confirmedBookings ?? []).filter(b => {
    // Время слота — московское (в нём живёт всё расписание), поэтому момент
    // собираем явным смещением, а не локальной зоной сервера: на Vercel она UTC.
    const начало = new Date(`${b.booking_date}T${b.start_time.length === 5 ? b.start_time + ':00' : b.start_time}+03:00`)
    return начало.getTime() < порогМс
  })
