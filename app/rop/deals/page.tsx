...
export default async function DealsPage({ searchParams }: { searchParams: Promise<Record<string, string>> }) {
  ...
  const [
    ...
    supabase.from('deals').select('id, title, stage_id, salesperson_id, source, budget, is_critical, updated_at, created_at, deleted_at').is('deleted_at', null),
    supabase.from('pipeline_stages').select('id, name, position, stage_type, color').eq('is_active', true).order('position'),
    supabase.from('users').select('id, name').eq('role', 'salesperson'),
