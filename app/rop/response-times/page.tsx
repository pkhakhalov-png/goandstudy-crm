...
export default async function ResponseTimesPage() {
  ...
  const [
    ...
    supabase.from('deal_messages').select('id, deal_id, direction, created_at').order('created_at'),
    supabase.from('deals').select('id, title, salesperson_id, stage_id, updated_at, deleted_at').is('deleted_at', null),
    supabase.from('pipeline_stages').select('id, name, stage_type').eq('is_active', true).order('position'),
