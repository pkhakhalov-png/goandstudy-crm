...
export function CollapsibleCard({ eyebrow, title, summary, chip, initiallyOpen = false, highlight = false, children }: Props) {
  ...
  return (
    <div
      ...
      style={{
        padding: 0,
        display: 'flex', flexDirection: 'column', height: 'fit-content', overflow: 'hidden',
    ...
    >
      <button
        ...
        style={{
          ...
          gap: 16, padding: '22px 28px', background: 'transparent', border: 'none',
          cursor: 'pointer', textAlign: 'left', fontFamily: 'var(--ds-font)', width: '100%',
        }}
      >
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, flexWrap: 'wrap' }}>
            <h3 style={{ fontFamily: 'var(--ds-font-display-stack)', fontWeight: 700, fontSize: 22, textTransform: 'uppercase', letterSpacing: '0.04em', color: 'var(--ds-ink)', margin: 0 }}>
              {title}
            </h3>
            {chip}
          </div>
          <div style={{ fontSize: 13, color: 'var(--ds-muted)', marginTop: 6, letterSpacing: '-0.005em' }}>
            {summary}
          </div>
        </div>
        ...
        <div style={{
          ...
          <span style={{ display: 'inline-block', transform: open ? 'rotate(180deg)' : 'rotate(0)', transition: 'transform 220ms ease-out', lineHeight: 1 }}>
            ▾
          </span>
        </div>
      </button>
