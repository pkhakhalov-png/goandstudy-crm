...
import { saveClientDraft, submitToCurator, curatorSaveEdit, approveEssay } from '@/app/demo/essays/actions'
...
export function ResumeEditor({ initialResume, clientId, status = 'draft', viewerRole }: ResumeEditorProps = {}) {
  ...
  // /demo читаем стартовое резюме из sessionStorage если оно там есть, иначе из demo.state.resume
  const [resume, setResume] = useState<Resume>(() => normalizeResume((initialResume ?? demo.state.resume as any) || INITIAL_RESUME))
  ...
  // Демо-синк: любое изменение резюме → пишем в DemoState (sessionStorage).
  useEffect(() => {
    demo.setResume(resume as any)
  ...
  function handleSubmit() {
    demo.setResume(resume as any)
    demo.setResumeStatus('sent')
    alert('Резюме отправлено куратору ✓')
  }
