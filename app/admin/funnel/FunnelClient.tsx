...
export function FunnelClient({ stages: serverStages, deals: serverDeals, salespersons, isAdmin, userId }: Props) {
  ...
  // Sync with server
  if (serverDeals !== localDeals && !dragDealId) {
    setLocalDeals(serverDeals)
  }
  if (serverStages !== localStages && !editingStageId && !colorPickerStageId) {
  ...
  async function handleDrop(stageId: string) {
    ...
    setLocalDeals(prev => prev.map(d =>
      d.id === dragDealId ? { ...d, stage_id: stageId, updated_at: new Date().toISOString() } : d
    ))
    resetDrag()

    // Server sync — background
    const fd = new FormData()
    fd.append('deal_id', dragDealId)
    fd.append('stage_id', stageId)
