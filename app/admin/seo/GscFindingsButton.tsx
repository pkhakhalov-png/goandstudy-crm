...
import { startGscFindings } from './actions'

export function GscFindingsButton() {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <button type="button" disabled={pending}
        onClick={() => start(async () => { const r = await startGscFindings(); setMsg(r.error || 'GSC-пересчёт запущен'); router.refresh() })}
