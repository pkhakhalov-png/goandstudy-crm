...
type FixedExpense = { id: string; name: string; amount: number; period: string; article: string; is_active: boolean }
...
const thStyle = { padding: '10px 20px', fontSize: 10, color: '#8a8796', fontWeight: 600 as const, textAlign: 'left' as const, textTransform: 'uppercase' as const, letterSpacing: '0.05em', background: '#F9F8FC' }
...
export function SettingsClient({ salespersons, curators, fixedExpenses }: {
  ...
  async function handleResetPassword(formData: FormData) {
    ...
    if (result.password) { setNewPasswordName(name); setNewPassword(result.password) }
  }

  const monthlyTotal = fixedExpenses.filter(e => e.is_active && e.period === 'monthly').reduce((s,e) => s + Number(e.amount), 0)
  const yearlyTotal = fixedExpenses.filter(e => e.is_active).reduce((s,e) => {
    if (e.period === 'monthly') return s + Number(e.amount) * 12
    if (e.period === 'quarterly') return s + Number(e.amount) * 4
    if (e.period === 'yearly') return s + Number(e.amount)
    return s
  }, 0)
    ...
    <div style={{ padding: '24px 28px', maxWidth: 860 }}>
      ...
      <div style={cardStyle}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr>
            <th style={thStyle}>Имя</th><th style={thStyle}>Email</th><th style={thStyle}>Статус</th><th style={{ ...thStyle, textAlign: 'right' as const }}></th>
          </tr></thead>
          <tbody>
            {salespersons.map(s => (
      ...
      <div style={cardStyle}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr>
            <th style={thStyle}>Имя</th><th style={thStyle}>Статус</th><th style={{ ...thStyle, textAlign: 'right' as const }}></th>
          </tr></thead>
          <tbody>
            {curators.map(c => (
      ...
      <div style={cardStyle}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(0,0,0,.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: '#8a8796', marginTop: 2 }}>Постоянные затраты не привязанные к клиентам</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontSize: 11, color: '#8a8796' }}>В месяц: <strong style={{ color: '#14121e' }}>{monthlyTotal.toLocaleString('ru')} ₽</strong></div>
            <div style={{ fontSize: 11, color: '#8a8796' }}>В год: <strong style={{ color: '#14121e' }}>{yearlyTotal.toLocaleString('ru')} ₽</strong></div>
          </div>
        </div>
        ...
        <form action={addFixedExpense} style={{ padding: '14px 20px', background: '#F9F8FC', borderBottom: '1px solid rgba(0,0,0,.07)' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr auto', gap: 8, alignItems: 'flex-end' }}>
            <div>
              ...
              <input name="name" required placeholder="Аренда офиса" style={{ ...inputStyle, flex: 'none', width: '100%' }} />
            </div>
            <div>
              <div style={{ fontSize: 10, color: '#8a8796', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.05em', marginBottom: 4 }}>Сумма *</div>
              <input name="amount" type="number" required placeholder="50000" style={{ ...inputStyle, flex: 'none', width: '100%' }} />
            </div>
            <div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {fixedExpenses.map(e => (
              <>
                <tr key={e.id} style={{ borderBottom: '1px solid rgba(0,0,0,.07)', opacity: e.is_active ? 1 : 0.5 }}>
                  <td style={{ padding: '12px 20px' }}>
                    {editingFixed === e.id ? (
                      <form action={async (fd) => { await updateFixedExpense(fd); setEditingFixed(null) }} style={{ display: 'flex', gap: 6, flexWrap: 'wrap' as const }}>
                        ...
                        <input name="name" defaultValue={e.name} style={{ ...inputStyle, flex: 'none', width: 160 }} autoFocus />
                        <input name="amount" type="number" defaultValue={e.amount} style={{ ...inputStyle, flex: 'none', width: 100 }} />
                  ...
                  <td style={{ padding: '12px 20px', fontSize: 13, fontWeight: 700 }}>{Number(e.amount).toLocaleString('ru')} ₽</td>
                  <td style={{ padding: '12px 20px' }}>
                    {e.is_active
            ...
            {fixedExpenses.length === 0 && <tr><td colSpan={6} style={{ padding: '20px', textAlign: 'center', fontSize: 12, color: '#8a8796' }}>Нет фиксированных расходов</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
